...
def get_sentiment(review: str, config: Config) -> dict:
    try:
        model_path, vocab_size, lookup_table_path, num_oov_buckets = read_env_vars(config)
...
def read_env_vars(config: Config):
    return config['MODEL_PATH'], config['VOCAB_SIZE'], config['LOOKUP_TABLE_PATH'], config['NUM_OOV_BUCKETS']
...
def create_review_preprocessor(lookup_table, vocab_size, num_oov_buckets) -> ReviewPreprocessor:
